...
int transfer_n(int sock,unsigned char S[],unsigned char Nn[],unsigned char *F,clock_t *rand_clock,clock_t *sock_clock);
...
int main(void) {
  ...
  while(cnt !=1000){
    ...
    s_clock = clock();
    //接続済のソケットでデータのやり取り
    transfer_n(c_sock,S,Nn,&F,&rand_clock,&sock_clock);
