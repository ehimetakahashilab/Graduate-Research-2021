...
int transfer_first(int sock,unsigned char A1[],unsigned char M1[],unsigned char An_prev[],unsigned char Mn_prev[]);
int transfer_n(int sock,unsigned char An[],unsigned char Mn[],unsigned char An_prev[],unsigned char Mn_prev[],clock_t *sock_clock);
...
int main(void) {
  ...
  unsigned char An_prev[BUF_SIZE] ={0};
  unsigned char Mn_prev[BUF_SIZE] ={0};
  ...
  transfer_first(sock,A1,M1,An_prev,Mn_prev);
  ...
  while(cnt !=1000){
    ...
    transfer_n(sock,An,Mn,An_prev,Mn_prev,&sock_clock);
...
int transfer_n(int sock,unsigned char An[],unsigned char Mn[],unsigned char An_prev[],unsigned char Mn_prev[],clock_t *sock_clock) {
  int recv_size, send_size,i;
  unsigned char recv_buf[BUF_SIZE+1]={0};
  ...
  // serverから文字列を受信
  recv_size = recv(sock, recv_buf, BUF_SIZE+1, 0);
  if (recv_size == -1) {
    printf("recv error\n");
    return 0;
  }
