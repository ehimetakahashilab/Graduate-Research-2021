...
conn = mysql.connector.connect(host="localhost", db="white_list", user="yasuhara", passwd="root", charset="utf8", auth_plugin="mysql_native_password")
            ...
            if sec > connect_sec:
                ...
                for key in _path_src_dst:
                    if len(_path_src_dst[key]) > WEB_SCANNING_THRESHOLD:
                        ...
                        out(_src_ip,_dst_ip,PROTO.TCP,packet)
                _path_src_dst.clear()
...
def out(src_ip,dst_ip,protocol,packet):
	...
	now = today.strftime('%Y-%m-%d %H:%M:%S.%f')
	...
	if direction == 0:
		...
		sql = "SELECT EXISTS(SELECT * FROM IPv4 WHERE ip=INET_ATON('"+src_ip+"') AND ip=INET_ATON('"+dst_ip+"'));"
